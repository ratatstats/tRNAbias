...
## ----transcriptomeCodonBias, fig.height=3.2, fig.width=8, fig.cap="\\label{fig:transcriptomeCodonBias}Codon usage bias introduced by the phage tRNA pool is more pronounced in late genes than early genes. (A) A multidimensional scaling (MDS) plot of phage and host genes using Shannon-Jensen Divergence of the codon distributions shows that codon usage difference between phage and host. Points representing the codon recognition capacities of the tRNA pool for each organism are overlaid. Points representing the average codon usage for each organism are also overlaid. The axes have been omitted, as, for an MDS plot, the main message is the relationship/distance of the points (phage or host genes) to each other. (B) Preference for the phage tRNA vs. the host tRNA pool (slant). (Zero signifies ambivalence.) (C) Mean codon usage for host and phage. (D) Slant toward the phage tRNA pool vs. timing of expression depicted as center of mass of RNA expression for the first round of infection. Note that this is different from the expression timing described in figure 1. The center of mass in this plot indicates how quickly the RNA transcript is degraded, while the time to half maximum expression shown in figure 1 summarizes transcription timing."----
...
## ----phageLifecycle, fig.height=6, fig.width=3.3, fig.cap="\\label{fig:phageLifecycle}Time course of phage 2.275.O. infection. (A) Phage plaques begin to appear 30 minutes into infection, and peaks at approximately 90 minutes. (B) Genomic qPCR results show that the host genome is degraded rapidly upon infection. Approximately a tenth of the hosts remain uninfected and begin to grow again. A second round of infection starts at approximately 90 minutes. (C) The tRNA subset of RNA-seq shows that host tRNA, shown in purple, are degraded upon infection while phage tRNA, shown in orange, rapidly increase. Reads are normalized to a firefly luciferase spike-in for each sample."----
